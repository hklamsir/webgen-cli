...
class FileGenerator:
    ...
    def scan_php_safety(content: str) -> list[str]:
        ...
        risky_funcs = [
            ('eval\s*\(', "禁止使用 eval()"),
            ('system\s*\(', "禁止使用 system()"),
            ('exec\s*\(', "禁止使用 exec()"),
            ('shell_exec\s*\(', "禁止使用 shell_exec()"),
            ('passthru\s*\(', "禁止使用 passthru()"),
            ('popen\s*\(', "禁止使用 popen()"),
            (r'file_get_contents\s*\(\s*["\']php://input',
             "禁止 php://input 直接讀取"),
        ]
        for pattern, msg in risky_funcs:
            if re.search(pattern, content, re.IGNORECASE):
                dangers.append(msg)

        # SQL 注入加強
        if re.search(r'SELECT.*\'.*\$_', content, re.IGNORECASE):
            dangers.append("可能存在 SQL 注入風險，建議使用 PDO 預處理語句")

        # XSS 加強
        if re.search(
                r'echo\s+["\'].*\$_GET', content,
                re.IGNORECASE) and not re.search(
                    r'htmlspecialchars|htmlentities', content, re.IGNORECASE):
            dangers.append("未過濾的 GET 參數輸出，可能導致 XSS")
    ...
    def _generate_and_write_file_worker(
            self, file_path_str: str, description: str, file_list: List[str],
            header_prompt: Optional[str], footer_prompt: Optional[str],
            project_path_str: str) -> tuple[str, str, str]:
        """
        (並行工作單元) 生成單一檔案內容。
        """
        try:
            ...
            content_response = llm_client.generate_file_content(
                description, file_path_str, file_list, self.config,
                header_prompt, footer_prompt)
            ...
            if not content_response:
                ...
                file_content = file_manager.extract_and_clean_code(
                    content_response, file_path_str)
                if not file_content:
                    file_content = ""
                    status = "extract_fail"
    ...
    def generate_project_files(
            self, project_path: Path, file_list: List[str], description: str,
            header_prompt: Optional[str],
            footer_prompt: Optional[str]) -> List[tuple[str, str]]:
        """
        步驟 2: 並行生成所有專案檔案。
        """
        max_workers = self.config.get("max_generation_workers",
                                      os.cpu_count() or 3)
        errors = []
        ...
        with concurrent.futures.ThreadPoolExecutor(
            ...
            futures = {
                executor.submit(self._generate_and_write_file_worker, f, description, file_list, header_prompt, footer_prompt, project_path_str):
                f
                for f in file_list
            }

            # (修正) 使用 with tqdm() as pbar，並呼叫 pbar.write() 來避免打斷進度條
            with tqdm(concurrent.futures.as_completed(futures),
                      total=len(file_list),
                      desc="生成進度",
                      unit="file",
                      ncols=80,
                ...
                for future in pbar:
                    ...
                    if status != "success":
                        ...
                        pbar.write(
                            f"{Fore.RED}[失敗] {fname}: {status}{Style.RESET_ALL}"
                        )

        print()  # <-- (修正) 在進度條結束後補上一個換行
        return errors

    def add_new_page(self, project_path: Path, file_path: str,
                     description: str, reference_context: str) -> bool:
        """
        ...
        resp = llm_client.generate_addpage_content(description, file_path,
                                                   self.config,
                                                   reference_context)
    ...
    def edit_existing_file(self, target_path: Path, file_path_str: str,
                           original_content: str, description: str,
                           reference_context: str,
                           structure: Optional[List[str]],
                           is_html_source: bool) -> bool:
        """
        處理 'edit' 命令的 AI 生成邏輯。
        """
        resp = llm_client.edit_file_content(file_path_str, original_content,
                                            description, self.config,
                                            structure, reference_context)

        if resp:
            if is_html_source:
                new_content = file_manager.extract_and_clean_code(
                    resp, file_path_str)
            else:
