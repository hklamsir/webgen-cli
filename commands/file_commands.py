...
class FileCommands:
    ...
    # --- 安全性輔助函數 ---
    def _is_safe_path(self: "ShellInterface", user_input: str, target_path: Path) -> bool:
        ...
        try:
            ...
            target_abs = (project_path / user_input).resolve()
    ...
    def do_dir(self: "ShellInterface", arg: str):
        """
        列出專案目錄結構。
        """
        if not self.current_session: 
    ...
    def do_rename(self: "ShellInterface", arg: str):
        """
    ...
    def do_delete(self: "ShellInterface", arg: str):
        """
        刪除檔案: delete <file>
        """
        if not self.current_session: 
    ...
    def do_merge(self: "ShellInterface", arg: str):
        """
    ...
    def do_updatehtml(self: "ShellInterface", arg: str):
        """手動重新組裝 HTML。"""
        if not self.current_session: 
            return print("請先開啟專案。")
        print("正在重新組裝...")
    ...
    def complete_rename(self: "ShellInterface", text: str, line: str, begidx: int, endidx: int) -> List[str]:
        return self._get_project_file_completions(text)

    def complete_delete(self: "ShellInterface", text: str, line: str, begidx: int, endidx: int) -> List[str]:
        return self._get_project_file_completions(text)

    def complete_merge(self: "ShellInterface", text: str, line: str, begidx: int, endidx: int) -> List[str]:
        return self._get_project_file_completions(text)